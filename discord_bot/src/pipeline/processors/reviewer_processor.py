...
def generate_reviewer_pool(
    ...
    # Preserve manual reviewers from existing config
    if github_org:
        mt_client = get_mt_client()
        existing_config = (
            mt_client.get_org_document(github_org, 'pr_config', 'reviewers') or {}
        )
    else:
    ...
    top_contributor_reviewers = [
        ...
        if data.get('stats', {}).get('pr', {}).get('all_time', data.get('pr_count', 0)) > 0
    ]
